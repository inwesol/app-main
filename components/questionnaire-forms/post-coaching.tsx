...
): PostCoachingFormData {
  const texts = questionTexts(questions);
  const expanded = { ...defaultValues } as PostCoachingFormData;
  texts.forEach((text, index) => {
    const key = `q${index + 1}`;
    const val = compact[key];
    if (typeof val === "number") {
      expanded[text as keyof PostCoachingFormData] = val;
    }
  });
  return expanded;
}
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (savedData?.answers && Object.keys(savedData.answers).length > 0) {
            ...
            // If stored in compact form (q1..qN), expand to form shape
