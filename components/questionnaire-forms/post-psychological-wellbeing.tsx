...
function PostPsychologicalWellbeing({ sessionId }: { sessionId: string }) {
  ...
  useEffect(() => {
    setQuestionnaireBreadcrumbs(sessionId, "Wellbeing Assessment-2");
  }, [sessionId, setQuestionnaireBreadcrumbs]);

  // Memoize defaultValues to prevent recreation on every render
  const defaultValues: PostPsychologicalWellbeingFormData = useMemo(
    () =>
      questions.reduce((acc, question) => {
  ...
  useEffect(() => {
    ...
    const fetchSavedData = async () => {
      const qId = "post-psychological-wellbeing";
      setIsLoading(true);
      try {
