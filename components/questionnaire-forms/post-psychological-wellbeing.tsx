...
const postPsychologicalWellbeingSchema = z.object(
  questions.reduce((acc, question) => {
    acc[question.text] = z.enum(allowedAnswers).optional();
    return acc;
  }, {} as Record<string, z.ZodTypeAny>)
);
type PostPsychologicalWellbeingFormData = z.infer<
  typeof postPsychologicalWellbeingSchema
>;
...
function PostPsychologicalWellbeing({ sessionId }: { sessionId: string }) {
  const [currentPage, setCurrentPage] = useState(0);
  const [isLoading, setIsLoading] = useState(true);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const router = useRouter();

  const defaultValues: PostPsychologicalWellbeingFormData = questions.reduce(
    (acc, question) => {
      acc[question.text as keyof PostPsychologicalWellbeingFormData] =
        undefined;
      return acc;
    },
    {} as PostPsychologicalWellbeingFormData
  );

  const form = useForm<PostPsychologicalWellbeingFormData>({
    resolver: zodResolver(postPsychologicalWellbeingSchema),
    defaultValues,
  });
    ...
    const fetchSavedData = async () => {
      ...
      try {
        const response = await fetch(
          `/api/journey/sessions/${sessionId}/q/post-psychological-wellbeing`
        );
        if (response.status === 404) {
          ...
          const savedData: Partial<PostPsychologicalWellbeingFormData> =
            await response.json();
  ...
  const onSubmit = async (data: PostPsychologicalWellbeingFormData) => {
    console.log("post-psychological-wellbeing data:", data);
  ...
  const getPageCompletionStatus = useCallback(
    (pageIndex: number) => {
      ...
      const answeredInPage = pageQuestions.filter((q) => {
        const fieldValue =
          watchedValues[q.text as keyof PostPsychologicalWellbeingFormData];
        return (
  ...
  const totalAnswered = questions.filter((question) => {
    const fieldValue =
      watchedValues[question.text as keyof PostPsychologicalWellbeingFormData];
      ...
      <div className="min-h-screen bg-gradient-to-br from-primary-green-50 via-white to-primary-blue-50 flex items-center justify-center p-4">
        <div className="text-center">
          ...
          <p className="text-slate-600 text-xs sm:text-base mt-2">
            Please wait while we prepare your post psychological wellbeing
            test...
          </p>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-green-50 p-4 sm:py-8">
      <div className="max-w-4xl mx-auto">
        ...
        <Header
          headerIcon={Brain}
          headerText="Post Psychological Well-being Assessment"
          headerDescription="Explore your psychological well-being across six key dimensions after your journey.
                ...
                <div className="space-y-6">
                  {currentPageData.questions.map((question) => (
                    <FormField
                      ...
                      name={
                        question.text as keyof PostPsychologicalWellbeingFormData
                      }
                      render={({ field }) => (
                        <FormItem>
...
export default PostPsychologicalWellbeing;
