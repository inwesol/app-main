...
import {
  preCoachingSdqSchema,
  sdqScoringConfig,
  impactScoringConfig,
} from "@/lib/form-validation-schemas/questionnaire-schemas/pre-coaching-sdq-schema";

type FormData = z.infer<typeof preCoachingSdqSchema>;
...
export default function PreCoachingSDQ({ sessionId }: { sessionId: string }) {
  ...
  const form = useForm<FormData>({
    resolver: zodResolver(preCoachingSdqSchema),
    defaultValues,
    mode: "onChange",
  });
  ...
  useEffect(() => {
    async function fetchData() {
      const qId = "pre-coaching-strength-difficulty";
      setIsLoading(true);
      try {
        const res = await fetch(`/api/journey/sessions/${sessionId}/q/${qId}`);
        if (res.ok) {
          const savedData = await res.json();
          console.log("Fetched SDQ data:", savedData);
          if (
            savedData &&
            ...
            console.log("Merged data for form:", mergedData);
            // Use setTimeout to ensure form is fully initialized before reset
            setTimeout(() => {
              reset(mergedData);
            }, 100);
          } else {
            console.log("No valid answers found, using default values");
            reset(defaultValues);
          }
        } else if (res.status === 404) {
          console.log("No data found (404), using default values");
          reset(defaultValues);
        } else {
        ...
        <Card className="max-w-lg mx-auto border-0 shadow-xl bg-white/95 backdrop-blur-sm">
          <CardContent className="p-8 text-center sm:p-12">
            ...
            <p className="text-base leading-relaxed text-slate-600 sm:text-lg">
              Your pre-coaching SDQ assessment has been submitted successfully.
          ...
          <div className="p-5 mb-6 border shadow-lg bg-gradient-to-br from-primary-blue-50 via-white to-primary-green-50 rounded-2xl sm:p-6 border-slate-200/60 backdrop-blur-sm">
            <div className="flex flex-col gap-3">
              ...
              <p className="text-base leading-relaxed text-slate-700">
                Below are some statements about how you think, and behave in
                different situations. Read each statement carefully and select
                the option that best indicates the extent to which you agree or
                disagree with the statement. There are no right or wrong
